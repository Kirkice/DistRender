...
// use crate::gfx::GfxDevice; // OLD
use crate::gfx::VulkanBackend as GfxDevice; // NEW Alias
...
impl Renderer {
    pub fn new(event_loop: &EventLoop<()>) -> Self {
        let gfx = GfxDevice::new(event_loop);
        ...
        let vs = vs::load(gfx.device.clone()).unwrap();
        let fs = fs::load(gfx.device.clone()).unwrap();
