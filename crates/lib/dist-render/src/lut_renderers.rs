use dist_render_backend::{ash::vk, vk_sync::AccessType, ImageDesc};
use dist_render_rg::{BindRgRef, IntoRenderPassPipelineBinding};

#[allow(unused_imports)]
use dist_render_backend::{ash::vk::ImageUsageFlags, vulkan::image::*};
...
impl ComputeImageLut for BrdfFgLutComputer {
    fn create(&mut self, device: &dist_render_backend::Device) -> dist_render_backend::Image {
        device
            .create_image(
    ...
    fn compute(
        &mut self,
        rg: &mut dist_render_rg::RenderGraph,
        img: &mut dist_render_rg::Handle<dist_render_backend::Image>,
    ) {
