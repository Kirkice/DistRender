...
impl LazyWorker for CompileRustShaderCrate {
    ...
    async fn run(self, ctx: RunContext) -> Self::Output {
        let src_dirs = || -> Result<_> {
            Ok([
                normalized_path_from_vfs("/dist-render/crates/lib/rust-shaders/src")?,
                normalized_path_from_vfs("/dist-render/crates/lib/rust-shaders-shared/src")?,
            ])
        };
...
fn compile_rust_shader_crate_thread(
    ...
    let builder_dir = normalized_path_from_vfs("/dist-render/crates/bin/rust-shader-builder")?;
