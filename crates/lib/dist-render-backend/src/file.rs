...
lazy_static! {
    static ref VFS_MOUNT_POINTS: Mutex<HashMap<String, PathBuf>> = Mutex::new(
        vec![
            ("/dist-render".to_owned(), PathBuf::from(".")),
...
pub fn set_standard_vfs_mount_points(dist_render_path: impl Into<PathBuf>) {
    let dist_render_path = dist_render_path.into();
    set_vfs_mount_point("/dist-render", &dist_render_path);
    set_vfs_mount_point("/shaders", dist_render_path.join("assets/shaders"));
    set_vfs_mount_point(
        "/rust-shaders-compiled",
        dist_render_path.join("assets/rust-shaders-compiled"),
    );
    set_vfs_mount_point("/images", dist_render_path.join("assets/images"));
}
