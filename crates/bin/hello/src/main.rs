use dist_render::world_renderer::AddMeshOptions;
use dist_render_simple::*;

fn main() -> anyhow::Result<()> {
    let mut dist_render = SimpleMainLoop::builder().resolution([1920, 1080]).build(
        WindowBuilder::new()
            .with_title("hello-dist-render")
    ...
    let lens = CameraLens {
        aspect_ratio: dist_render.window_aspect_ratio(),
        ..Default::default()
    };

    let car_mesh = dist_render
        .world_renderer
        .add_baked_mesh("/cache/336_lrm.mesh", AddMeshOptions::new())?;

    let car_inst = dist_render.world_renderer.add_instance(
        car_mesh,
