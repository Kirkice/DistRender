...
Instead, Dist Render uses a low-resolution irradiance cache. It's stored as a set of 12 camera-aligned sparsely-allocated 32x32x32 clip maps -- meaning that there's a dense top-level 32x32x32x12 indirection array which indexes into a set of payload buffers pre-allocated to a max number (65536) of entries.
...
At present, Dist Render doesn't have spatial reservoir exchange for reflections, but it will certainly come in handy for rough surfaces. Even then, the temporal part alone helps tremendously with smooth and mid-rough materials.
...
They're denoised using a slightly modified version of AMD's [FidelityFX Shadow Denoiser](https://gpuopen.com/fidelityfx-denoiser/). The changes are primarily about integrating it with Dist Render's temporal reprojection pipeline -- using a shared reprojection map instead of recalculating it from scratch.
...
Dist Render uses screen-space ambient occlusion, but not for directly modulating any lighting. Instead, the AO informs certain passes, e.g. as a cross-bilateral guide in indirect diffuse denoising, and for determining the kernel radius in spatial reservoir resampling.
...
Note that normally, Dist Render uses very little in terms of spatial filtering, but it's forced to do it when ReSTIR reservoirs are starved for samples (e.g. upon camera jumps). If we force the spatial filters to actually run, the difference is a lot more pronounced.
...
Dist Render's own performance counters averaged over 30 frames; note that there is some overlap between passes, making this not entirely accurate:
...
[final dist render frame]: https://user-images.githubusercontent.com/16522064/171687011-e79fdb79-64fe-4f7d-a9a0-7146975c3a1d.png
