...
DLSS is supported on Nvidia RTX GPUs, and Dist Render can currently use it when running on Windows.
...
Therefore, the easiest way to get DLSS into Dist Render is to [download the UE5 DLSS plugin](https://developer.nvidia.com/dlss-getting-started#ue-version), and extract the following:

* Copy `DLSS/Binaries/ThirdParty/Win64/nvngx_dlss.dll` to the project root folder (where this README resides).
...
When building Dist Render, use the `dlss` Cargo feature, and specify temporal upsampling, e.g.:
...
Please note that while DLSS works well for AAA-style content, it currently struggles with flat colors and smooth gradients. Dist Render's built-in TAA and temporal upsampling tends to look better there.
